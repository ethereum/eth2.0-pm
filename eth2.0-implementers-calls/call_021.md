...
### Meeting Date/Time: Thursday July 11, 2019 at 14:00 GMT
### Meeting Duration 1.5 hours
### [GitHub Agenda Page](https://github.com/ethereum/eth2.0-pm/issues/55)
### [Youtube Live Steam Link](https://youtu.be/YB8o_5qjNBc)
...
# 1. Testing Updates
[Timestamp 4:05](https://youtu.be/YB8o_5qjNBc?t=245)

**Danny Ryan**:
* We will get started with some testing updates, as usual
...
* Relatively non-substantive changes (fixes, documentation, and things) in `v08x` branch [#1286](https://github.com/ethereum/eth2.0-specs/pull/1286)
    * Includes a fix to this which alters the way the computation happens. Non-substantive, but doesn't overflow
* Seems relatively important so will get out `v08x` in the next few days (includes other minor things people have found).
* Any updates Proto?

**Diederik Loerakker**
* Spec Freeze is over now, can stop throwing up to the spec and actually build.
...
* I think the next target for integration will be Lighthouse.
    * Hopefully a lot of the infrastructure will be generic for clients.
* Might come in the next two weeks, hopefully.

**Danny Ryan**:
* Cool, I guess we'll go the experiment round with one and learn some things. Entire 107.
...
* It's really up to the losts, like the first clients, to also be on the same spec version. Ain't gonna start so soon.

**Jacek Sieka**:
* About the Fuzzing, how do you drive it?
* Like how is the Fuzzing driven, what is the input?

**Diederik Loerakker**:
* The problem with Fuzzing is that we have 2 kinds of inputs:
    1. States
    2. Blocks
* The State itself is really easily randomized. It contains script traffic contents. It has all these validity assumptions.
* You can have any kind of Block, these are like the attack vector you could say. So you're Fuzzing blocks now and you're extending at seconds corpora echoes of the pre-state.
* Generally you could say we are mostly focusing on Blocks now.
* They are looking to make this extension of the State more intelligent so that you could actually make more progress in a chain test review of more different pre-states being Fuzzed.
...
**Paul Hauner**:
* Yeah, I'm not exactly sure how the EF (Ethereum Foundation) one works but I know May's explained how our one works
    * You start with a valid block or some valid object that kind of reaches as far into code path as it can
    * Then the Fuzzer meters all your code
    * Then it randomizes the fields of the original object you gave it
    * Then detects if it explores new code paths
...
* We don't want to involve only the block inputs because if you're always Fuzzing the same states, you don't really get the same coverage as you could if you're Fuzzing multiple states
...
* I think with PySpec is that there are these functions that are not really pleasant outs they all inefficiently repeat themselves, not precomputing anything
* Now in the next Go spec updates, I optimize, precompute this, precompute that, so we get half, almost client speed spec.
* I tried to keep this really portable.

**Danny Ryan**:
* One of the paths might be to differentially Fuzz against the Go spec and the clients and then if we find issues, to then go back to the Py spec and make sure we're in agreement on what the functionality should be.
* Okay, let's move on Fuzzing, thank you. 
* We'll keep y'all updated over the coming weeks on that.
...
* We also published metrics library for Prometheus compact metrics, I'm adding the link to the chat as well.
    * [Nim metrics client library supporting the Prometheus monitoring toolkit](https://github.com/status-im/nim-metrics)
* We have a EWASM research library and we are very happy with. We have started a domain specific language in NIM that compiles to EWASM. It's quite competitive in terms of contract size, so I'm posting the link as well.
...
# 3. Research Updates
[Timestamp 28:45](https://youtu.be/YB8o_5qjNBc?t=1725)
...
* Phase 1 stuff:
    ...
    * But still some decision to make around what the committees are for each individual slot and how those committees change
...
* To add on Phase 1, we're likely to have some cryptanalysis done
    * Kovatovich do an analysis of the cryptography that used there
...
* In parallel to Phase 1 & Phase 2, the research team is doing is more education about Phase 0
    ...
    * On July 15th 1:00PM GMT there will be a 2nd Ethereum 2.0 AMA, great opportunity to ask and answer questions related to your implementation work
...
* Posted first-half on Ethereum Research yesterday and we'll be posting the second-half and the applicability to state schemes and Ethereum 2 and multi-shard behavior and what that can open up.
    * Open to feedback on this as there are some cool applications there.
...
* Team is growing. John Adler is collabing with us. Trying to grow for Rust-based researchers. Trying to do more research on Phase 1/Phase 2 in parallel to expand a lot of this.
...
* Done the migration to v0.8 and looking to have something testable.
* There's an abstract model, but that's a lower priority type of development at the moment, but helpful for the future.
    * K model bazooka chain

# 4. Networking 
[Timestamp 41:10](https://youtu.be/YB8o_5qjNBc?t=2470)
**Mike Goelzer**
* Couple of updates:
...
* What I wanted to check with the folks on this call, are there any concerns on JS libp2p that we should be addressing, either us or chain safe. 
    ...
    * JS libp2p has been in production on IPFS, so it's not totally untested. But I do understand that Ethereum might have different requirements.
...
* Raúl, did you want to add anything?
...
* Discussion with some people that are doing an audit on the protocol.
* Any other movement on networking things people want to discuss?
...
[Timestamp 1:24:55](https://youtu.be/YB8o_5qjNBc?t=5095)
...
* This was done:
    ...
    2. Was also done in an effort to provide a valuable set of types in SSE that might be used in other places, such as the application area.
...
* Obviously there are some issues, so I want to open it up for a quick decision so that we can fix it if we need to.
...
**Vitalik Buterin**:
* Well, we already made them illegal. So we're basically revisiting that decision.
...
[Timestamp 1:32:20](https://youtu.be/YB8o_5qjNBc?t=5540)

**Danny Ryan**:
* Any other things before we close today?

**Joseph Delong**:
* For the interop, I just wanna say that the invitations went out to team leads. 
* Those invitations are good until the 14th.
* And for the Antoine's in the call, that's Bastille Day.
...
* Okay, thank you. A lot of really good discussion. 
...
* Steven Schroeder (ConsenSys/PegaSys)
* Trenton Van Epps
* Vitalik Buterin (EF/Research)
* Wei Tang (Parity)
* Whiteblock
* Will Villanueva (ConsenSys)
* Zak Cole
